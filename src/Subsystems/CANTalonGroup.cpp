...
double CANTalonGroup::Get() const { return m_canTalons[0].get().GetSelectedSensorPosition(m_canTalons[0].get().GetDeviceID()); }
...
void CANTalonGroup::Disable() { m_canTalons[0].get().Set(ControlMode::PercentOutput, 0.0); }
...
double CANTalonGroup::GetPosition() const {
    return m_canTalons[0].get().GetSelectedSensorPosition(m_canTalons[0].get().GetDeviceID()) * m_distancePerPulse;
}

double CANTalonGroup::GetSpeed() const {
    // RPM * degrees/rev / (seconds/min)
    return m_canTalons[0].get().GetSelectedSensorVelocity(m_canTalons[0].get().GetDeviceID()) * m_distancePerPulse / 60.0;
}
...
void CANTalonGroup::SetFeedbackDevice(FeedbackDevice device) {
    ...
    m_canTalons[0].get().ConfigSelectedFeedbackSensor(device, m_canTalons[0].get().GetDeviceID(), 50);
}

void CANTalonGroup::ResetEncoder() { m_canTalons[0].get().SetSelectedSensorPosition(0, m_canTalons[0].get().GetDeviceID(), 50); }
