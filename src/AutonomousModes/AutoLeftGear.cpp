    ...
    while (IsAutonomous() && IsEnabled() && !SMHasRun) {
        ...
        switch (state) {
            case State::Idle:
                ...
                std::cout << "Idle Started (Pos):" << robotDrive.GetPosition()
                          << std::endl;
                robotDrive.SetPositionReference(114.3 - 39 /*robot length*/);
                state = State::InitForward;
                std::cout << "Idle Ended (PosRef):"
                          << robotDrive.GetPosReference() << std::endl;
                break;

            // Initial Forward
            case State::InitForward:
                std::cout << "Position" << robotDrive.GetPosition()
                          << std::endl;
                if (robotDrive.PosAtReference()) {
                    robotDrive.SetAngleReference(315.0);
                    state = State::Rotate;
                    std::cout << "InitForward Ended (Pos):"
                              << robotDrive.GetPosition() << std::endl;
                }
                break;

            // Rotate
            case State::Rotate:
                std::cout << "Rotate Started(Ang)" << robotDrive.GetAngle()
                          << std::endl;
