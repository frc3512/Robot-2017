 ...
 */
class LinearDigitalFilter : public NodeBase {
public:
    LinearDigitalFilter(NodeBase* input, std::initializer_list<double> ffGains,
                        std::initializer_list<double> fbGains);
    LinearDigitalFilter(NodeBase* input, std::initializer_list<double> ffGains,
                        const std::vector<double>& fbGains);
    LinearDigitalFilter(NodeBase* input, const std::vector<double>& ffGains,
                        std::initializer_list<double> fbGains);
    LinearDigitalFilter(NodeBase* input, const std::vector<double>& ffGains,
                        const std::vector<double>& fbGains);

    // Static methods to create commonly used filters
    static LinearDigitalFilter SinglePoleIIR(NodeBase* input,
                                             double timeConstant,
                                             double period);
    static LinearDigitalFilter HighPass(NodeBase* input, double timeConstant,
                                        double period);
    static LinearDigitalFilter MovingAverage(NodeBase* input, int taps);
...
private:
    ...
    frc::CircularBuffer<double> m_inputs;
    frc::CircularBuffer<double> m_outputs;
