...
class LinearFilter : public NodeBase {
 public:
  LinearFilter(INode& input, std::initializer_list<double> ffGains,
               std::initializer_list<double> fbGains);
  LinearFilter(INode& input, std::initializer_list<double> ffGains,
               const std::vector<double>& fbGains);
  LinearFilter(INode& input, const std::vector<double>& ffGains,
               std::initializer_list<double> fbGains);
  LinearFilter(INode& input, const std::vector<double>& ffGains,
               const std::vector<double>& fbGains);

  // Static methods to create commonly used filters
  static LinearFilter SinglePoleIIR(INode& input, double timeConstant,
                                    double period);
  static LinearFilter HighPass(INode& input, double timeConstant,
                               double period);
  static LinearFilter MovingAverage(INode& input, int taps);
